...
const SinglePostMain = () => {
  ...
  const categorySlug = param.categoryName;
  const subCategorySlug  = param.slug.length>1 ? param.slug[0] :null;
  const postId: string   = param.slug.length>1 ? param.slug[1] :param.slug[0];
  //console.log(categorySlug, subCategorySlug, postId, param);
  /**
   * Fetch Data from API.
   *
