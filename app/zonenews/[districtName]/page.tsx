...
export default function ZoneNewsPage({ params }: { params: ZoneNewsParams }) {
  ...
  const handleUpazillaClick = (upazilla: { id: string; bn_name: string }) => {
    router.push(
      `/zonenews/${encodeURIComponent(divisionName || "")}/${encodeURIComponent(districtName || "")}/${encodeURIComponent(upazilla.bn_name)}?division_id=${divisionId}&district_id=${districtId}&upazilla_id=${upazilla.id}`
    );
  };
